...
exports.ComputerObservation = zod_1.z.object({ generation: zod_1.z.string().min(1), application: zod_1.z.string(), controls: zod_1.z.array(Control).max(150), truncated: zod_1.z.boolean(), apps: zod_1.z.array(zod_1.z.object({ id: zod_1.z.string(), name: zod_1.z.string() })).max(100) });
...
async function runComputerUse(raw, deps, signal) {
    ...
    try {
        ...
        return await (0, jev_loop_1.runLoop)({
            ...
            observe: async () => { check(); update(); last = exports.ComputerObservation.parse(await call('computer_observe')); return last; },
            decide: async (state) => {
                check();
